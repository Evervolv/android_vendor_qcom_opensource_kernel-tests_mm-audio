	...
	if (ioctl(afd, AUDIO_START, 0) >= 0) {
		for (;;) {
			...
			if (write(afd, buf, sz) != sz) {
				...
				cntW++;
				printf(" aac_play: instance=%d cntW=%d\n", (int) audio_data, cntW);
			}
		}
...
int aac_rec(struct audtest_config *config)
{
  ...
  rec_stop = 0;
   while (!rec_stop) {
    ...
    write(fd,audaac_header,AUDAAC_MAX_ADTS_HEADER_LENGTH);  // writing aac adts header format with frame len
...
"Play aac file: type \n \
echo \"playaac path_of_file -type=xxxx -rate=xxxx -cmode=x \
-profile=xxx -id=xxx -mode=x -bitstream=xxx -out=path_of_outfile\" > %s \n \
...
echo \"recaac path_of_file -id=xxx -rate=xxxx -cmode=x -bps=xx\" > %s \n\
